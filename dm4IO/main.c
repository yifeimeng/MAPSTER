...
#define DM4_FILE_NAME "/run/user/1000/gvfs/smb-share:server=mapster.lbl.gov,share=mapster_volume_1/NCEM_K2/HaoYang/Capture1.dm4"
...
void raw2HDF5(int64_t *targetTagsPositions) {
    ...
    uint32_t readerBuffer[READER_BUFFER_SCALE][totalNumberImages];
    ...
    uint32_t *writerBuffer = (uint32_t *)malloc(NUM_WRITER*numberImagePerWriter*WRITER_BUFFER_SCALE*4);
    
    //move the file pointer to the starting position of the rawData
    fseek(fp, targetTagsPositions[0] + 36, SEEK_SET);//another magic number
    ...
    for (uint64_t readIndex = 0; readIndex < numberReads; readIndex ++) {
        //read a chunk of the data into the buffer
        fread(readerBuffer, 4, totalNumberImages*READER_BUFFER_SCALE, fp);//mods
        ...
        uint64_t writerIndex, imageIndex, bi_writer;
        for (uint64_t bi = 0; bi < READER_BUFFER_SCALE; bi ++){
            for (uint64_t si = 0; si < totalNumberImages; si ++) {
          
                    writerIndex = si / numberImagePerWriter;
                    imageIndex = si % numberImagePerWriter;
                    uint64_t ptrPosition = writerIndex * NUM_WRITER + imageIndex * numberImagePerWriter + writerFiller * READER_BUFFER_SCALE + bi;
        ...
        if ((writerFiller + 1) == numberClearWriter) {
            ...
            for (uint64_t writerIndex = 0; writerIndex < NUM_WRITER; writerIndex ++) {//mods
                for (uint64_t imageIndex = 0; imageIndex < numberImagePerWriter; imageIndex ++) {
                    ...
                    uint32_t sdata[WRITER_BUFFER_SCALE];
                    uint64_t ptrShift = writerIndex * NUM_WRITER + imageIndex * numberImagePerWriter + fileFiller * WRITER_BUFFER_SCALE;
                    memcpy(sdata, (writerBuffer + ptrShift), WRITER_BUFFER_SCALE);
