...
void readTagData(FILE *fp, uint64_t *infoArray, uint64_t infoArraySize) {
    ...
    switch (tagType) {
        ...
        case 0x05: {//the tag is a 4 bytes uint32_integer
            uint32_t currValue;
            fread(&currValue, 4, 1, fp);
            printf("%d",currValue);
            break;
...
void readTagEntry(FILE *fp, uint64_t tagIndex, uint32_t tagLevel, char *tagLabel, char **targetTagsLabels, uint16_t numberTargetTags, int64_t *targetTagsPositions) {
    ...
    if (tagType == 0x14) {
        readTagGroup(fp, tagLevel, tagLabel, targetTagsLabels, numberTargetTags, targetTagsPositions);
    }
...
void readTagGroup(FILE *fp, uint32_t tagLevel, char *tagLabel, char **targetTagsLabels, uint16_t numberTargetTags, int64_t *targetTagsPositions) {
    ...
    for (uint64_t tagIndex = 0;tagIndex < numberTags;tagIndex++) {
        //printf("tag level is %d.\n",tagLevel);
        readTagEntry(fp, tagIndex, tagLevel, tagLabel, targetTagsLabels, numberTargetTags, targetTagsPositions);
    }
}
...
void pinpointTagsDM4(FILE *fp, int64_t *targetTagsPositions, char **targetTagsLabels, uint16_t numberTargetTags) {
    ...
    readTagGroup(fp, initialTagLevel, tagLabel, targetTagsLabels, numberTargetTags, targetTagsPositions);
