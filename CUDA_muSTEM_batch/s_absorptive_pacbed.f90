    subroutine absorptive_pacbed
        ...
        real(fp_kind),dimension(nopiy, nopix, n_cells*n_slices/8) :: pacbed_pattern,fourDSTEM_pattern
...
901         format(a1, 1x, 'y:', i3, '/', i3, ' x:', i3, '/', i3, ' Intensity: ', f12.6)
            ...
            call make_stem_wfn(psi, 0.0_fp_kind, probe_positions(:,ny,nx))
        ...
        do i_thickness = 1, n_thickness
            write(thickness_label, '(I3.3)') i_thickness
            thickness_label = adjustl(thickness_label)
            filename = trim(adjustl(output_prefix)) // '_z' // thickness_label
            call binary_out_unwrap(nopiy,nopix,pacbed_pattern(:, :, i_thickness),filename)
        enddo
