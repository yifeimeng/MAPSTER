module m_potential
    ...
    subroutine precalculate_scattering_factors
        ...
        real(fp_kind),allocatable :: tdsbrcoeff(:,:), xdata(:) 
        ...
        if(complex_absorption) then
            ...
            do k = 1, nt
                call spline(xdata, tdsbr(:,k), max_int, 1.0d31, 1.0d31, tdsbrcoeff(:,k))
            enddo
	    ...
	    do i = 1, nopiy
	        ...
	        do j = 1, nopix
                ...
                do k = 1, nt
                    ...
                    if(complex_absorption) then
                        call splint(xdata, tdsbr(:,k), tdsbrcoeff(:,k), max_int, sqrt(g2), temp)
    ...
    subroutine make_site_factor_hybrid(site_factor, tau)
        ...
        do j = 1, size(tau, 2)
            ...
            if (ceiling(xpos).ge.nopix) then
                xpos = xpos - float(nopix)
            elseif (floor(xpos).lt.1) then
                xpos = xpos + float(nopix)
            endif
            
            if (ceiling(ypos).ge.nopiy) then
